...
The initial set of features chosen based on past literature and common sense is outlined in the table below. (Dominic et. Al, 1996), (David, Krueger, 1996) 
...
Justification: For Alpha and Sigma, weekly informative priors were selected as
we did not posses enough information.
...
Justifications for the hyperpriors: When selecting hyperpriors it was considered
that the constructions would be consistent in a sense that there would not 
be conflicting levels of informativeness.
...
```{r, echo=FALSE, results = "hide", warning=FALSE, echo=FALSE}
...
All our priors strived to be weakly informative but not too vague.  We experimented with three different prior specifications: our conservative estimate for a good set of priors as well as two sets of priors set to be either wider or narrower than the conservative estimate. OUR RESULTS TURNED OUT (NOT) TO BE SENSITIVE TO DIFFERENT PRIOR SPECIFICATIONS.
...
EN TIEDÄ MITÄÄN K-HATISTA

As we have been building a predictive model, we though it would be important to perform predictive performance assessment. We use RMSE to assess the predictive ability of our model. Our data had been initially split into a training set and a test set. MITÄ TULOKSIA?
