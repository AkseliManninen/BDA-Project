---
title: "Bayesian Multivariate Regression on Income of University Alumni in the U.S."
author: "Niko Miller, Akseli Manninen, Santeri Löppönen"
output:
  pdf_document:
    toc: yes
    toc_depth: 1
...
# read in data sets
data <- vroom("./Data/Most-Recent-Cohorts-Institution.zip") %>%
  as_tibble()

data.description <- vroom("./Data/CollegeScorecardDataDictionary.zip") %>%
  as_tibble()
...
data.numerical <- data %>%
  ...
  mutate(across(.cols = c(id.vars[1], all_of(numerical.vars)), .fns = as.numeric))
...
The panel of figures below show scatter plots between the dependent variable and all numerical features. The x-axis represents the value of the feature and the y-axis represents the value of the dependent variable.
...
#### Summary of Numerical Feature Selection
...
ggsave("./Figures/categorical1.png")
...
ggsave("./Figures/categorical2.png")
...
ggsave("./Figures/categorical3.png")
...
The median individual income in the US was approximately 31 000 in 2020 (Data Commons, 2020). The average SAT score in the US in 2020 were 523 in Math
...
Sigma ~ Normal(10000, 10000)

Justification: For Alpha and Sigma, weekly informative priors were selected as we did not posses enough information.

Hyperpriors:

$Alpha_p$ ~ Normal(100, 100)

$SAT_p$ ~ Normal(100, 100)

$MD\_FAMINIC$ ~ Normal(10, 100)

$COSTT4\_A_p$ ~ Normal(50, 500)
                                
$POVERTY\_RATE_p$ ~ Normal(500, 500)
                                
$URBAN_p$ ~ Normal(500, 500)

$PRIVATE_p$ ~ Normal(500, 500)

$Sigma$~ Normal(500, 500)
...
separate.model.data <- list(N = nrow(data.joined.stan),
                            N_tilde = nrow(data.joined.stan.test),
                            K = length(unique(data.joined.stan$REGION)),
                            
                            x = data.joined.stan$REGION,
                            x_tilde = data.joined.stan.test$REGION,
                            ...
                            SAT_ALL_test = data.joined.stan.test$SAT_ALL,
                            MD_FAMINIC_test = data.joined.stan.test$MD_FAMINC,
                            COSTT4_A_test = data.joined.stan.test$COSTT4_A,
                            POVERTY_RATE_test = data.joined.stan.test$POVERTY_RATE,
                            URBAN_test = data.joined.stan.test$URBAN,
                            PRIVATE_test = data.joined.stan.test$PRIVATE,
                            y_test = data.joined.stan.test$MD_EARN_WNE_P10,
...
separate.fit$print(max_rows=73)
...
hierarchical.fit$print(max_rows=81)
...
model the ESS values range from 340 to 1225 (N = 766).
...
y_rep_separate <- matrix(data = separate.extract[,1,], nrow = 1000)

# Pooled

pooled.extract <- pooled.fit$draws(variable = "y_rep")

y_rep_pooled <- matrix(data = pooled.extract[,1,], nrow = 1000)

# Hierarchical

hierarchical.extract <- hierarchical.fit$draws(variable = "y_rep")

y_rep_hierarchical <- matrix(data = hierarchical.extract[,1,], nrow = 1000)
...
ppc_dens_overlay(y, y_rep_separate) + separate.ppctitle
ggsave("./Figures/post-pred-check-sep.png")
...
pooled.ppctitle <- ggtitle("Pooled model",
                             "Comparing densities of y and y_rep")
ppc_dens_overlay(y, y_rep_pooled) + pooled.ppctitle
ggsave("./Figures/post-pred-check-pooled.png")
...
hierarchical.ppctitle <- ggtitle("Hierarchical model",
                             "Comparing densities of y and y_rep")
ppc_dens_overlay(y, y_rep_hierarchical) + hierarchical.ppctitle
ggsave("./Figures/post-pred-check-hier.png")
...
separate.fit$print(variables = "y_tilde", max_rows = nrow(data.joined.stan.test))
separate.fit$print(variables = "indiv_squared_errors", max_rows = nrow(data.joined.stan.test))
separate.fit$print(variables = "sum_of_squares")
separate.fit$print(variables = "root_mean_squared_error")

color_scheme_set("blue")
bayesplot::mcmc_hist(x = separate.fit$draws(), pars = "root_mean_squared_error")
ggsave("./Figures/ypred-mcmc-hist-sep.png")
...
pooled.fit$print(variables = "y_tilde", max_rows = nrow(data.joined.stan.test))
pooled.fit$print(variables = "indiv_squared_errors", max_rows = nrow(data.joined.stan.test))
pooled.fit$print(variables = "sum_of_squares")
pooled.fit$print(variables = "root_mean_squared_error")

color_scheme_set("blue")
bayesplot::mcmc_hist(x = pooled.fit$draws(), pars = "root_mean_squared_error")
ggsave("./Figures/ypred-mcmc-hist-pooled.png")
...
hierarchical.fit$print(variables = "y_tilde", max_rows = nrow(data.joined.stan.test))
hierarchical.fit$print(variables = "indiv_squared_errors", max_rows = nrow(data.joined.stan.test))
hierarchical.fit$print(variables = "sum_of_squares")
hierarchical.fit$print(variables = "root_mean_squared_error")

color_scheme_set("blue")
bayesplot::mcmc_hist(x = hierarchical.fit$draws(), pars = "root_mean_squared_error")
ggsave("./Figures/ypred-mcmc-hist-hier.png")
