...
SAT.data <- 
  data.joined.stan %>% 
  select(REGION, SAT_ALL) %>%
  arrange(REGION) %>% 
  group_by(REGION) %>%
  mutate(row = row_number()) %>%
  pivot_wider(names_from = REGION, values_from = SAT_ALL) %>%
  select(-row)


separate.model <- cmdstan_model(stan_file = "./Stan/separate.mod.stan")
